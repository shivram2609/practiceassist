...
module.exports = {
			...
			var companyDetails = await Companies.create(user).fetch();
					...
					Users.create(companyDetail).then(function() {
						...
						return res.json({
							status: true,
							response: {},
							messages: ['Firm registered successfully.']
						});
    ...
    edit_user: async function (req, res) {
		...
		var UsersList = await Users.findOne({id: getCode});
		return res.json({ status: true, response: UsersList , messages:[getCode] });
    ...
    delete_user: async function (req, res) {
		...
		await Users.destroyOne({id: did});
        return res.json({ status: true, response: {}, messages: ["User deleted successfully."] });	
	},
        ...
        Users.findOne({
            ...
            return Users.comparePassword(current_password, user).then(function(valid) {
				 ...
				 Users.updateOne({
                    ...
                    .set({ password : passwords }).then(function(user) {
                        return res.json({
                            status: true,
                            response: {},
                            messages: ["Your password has been updated successfully."]
    ...
    update_status: async function(req , res) {
		...
		var get_user_status =  await Users.findOne({id: statusId});
		if(get_user_status.status == true) {
			...
			Users.updateOne({
                    ...
                    .set({ status : statusValue }).then(function(user) {
                        return res.json({
                            status: true,
                            response: {},
                            messages: [message]
